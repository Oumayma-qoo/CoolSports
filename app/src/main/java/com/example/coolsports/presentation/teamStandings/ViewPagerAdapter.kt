...
class ViewPagerAdapter(val fragment: Fragment,val rules: LeagueData04,  var leagueStanding: List<LeagueStandingsBase>, val leagueStanding2: List<LeagueStandingsGroupBase>, val leagueId: Int) : FragmentStateAdapter(fragment){
    ...
    override fun createFragment(position: Int): Fragment {
        return when(position){
            0 -> TeamStandingsFragment(rules,leagueStanding, leagueStanding2,leagueId)
            1 -> PlayerStandingsFragment(leagueId)
            else -> TeamStandingsFragment(rules,leagueStanding, leagueStanding2 ,leagueId)
        }
    }
